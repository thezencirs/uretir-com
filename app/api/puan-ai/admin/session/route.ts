...
export async function POST(request: NextRequest) {
  ...
  if (!isAdminConfigured()) return NextResponse.json({ error: "Admin erişimi yapılandırılmamış." }, { status: 503 });
  const parsed = loginSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success || !verifyAdminPassword(parsed.data.password)) {
    return NextResponse.json({ error: "Parola doğrulanamadı." }, { status: 401 });
  }
