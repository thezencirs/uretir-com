  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      try {
        ...
        const submittedUrl = payload.data.message.match(/https:\/\/[^\s]+/i)?.[0]?.replace(/[),.;]+$/, "");
        const urlVerification = submittedUrl ? await verifyCampaignUrl(submittedUrl) : null;
        const campaigns = submittedUrl ? [] : await searchVerifiedCampaigns(groundedQuery);
        controller.enqueue(encodeEvent({ type: "meta", conversationId, campaigns }));

        let answer = "";
        if (urlVerification) {
          const heading = urlVerification.status === "VERIFIED" ? "Doğrulandı" : "Doğrulanamadı";
          answer = `${heading}. ${urlVerification.reason}${urlVerification.title ? ` Kaynak başlığı: ${urlVerification.title}.` : ""}${urlVerification.validUntil ? ` Bitiş: ${new Date(urlVerification.validUntil).toLocaleDateString("tr-TR", { timeZone: "Europe/Istanbul" })}.` : ""}`;
