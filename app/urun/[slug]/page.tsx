...
import { getProductEntry as getProduct } from "@/lib/product-entry";
export const dynamic = "force-dynamic";
export function generateStaticParams() { return products.map(({ slug }) => ({ slug })); }


export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params; const entry = await getProduct(slug);
  return entry ? { title: entry.product.name + " — " + entry.maker.name, description: entry.product.description, alternates: { canonical: "/urun/" + entry.product.slug }, openGraph: { url: "/urun/" + entry.product.slug } } : {};
}
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const entry = await getProduct(slug);
  if (!entry) notFound();
  const { product: p, maker: m } = entry;
  ...
  return <div className="section-wrap py-14 md:py-24">
    ...
    <div className="mt-10 grid gap-10 md:grid-cols-[1.3fr_.7fr]">
      <div><p className="eyebrow">{p.category} / {catalogText(p.task, false)}</p><h1 className="mt-5 text-5xl font-bold tracking-tight md:text-7xl">{p.name}</h1><p className="mt-6 max-w-xl text-xl leading-8 text-muted">{catalogText(p.description, false)}</p><p className="mt-6">{m.name} · {m.city}, Türkiye</p><a className="mt-8 inline-flex rounded-lg bg-[#28452f] px-6 py-4 text-sm text-white" href={p.url} target="_blank" rel="noreferrer">Resmî ürün kataloğu / mağaza bağlantıları ↗</a><p className="mt-3 text-xs text-muted">İndirme, fiyat ve platform bilgilerini üreticinin sayfasından kontrol edebilirsin.</p></div>
      <aside className="rounded-xl border hairline p-7"><h2 className="text-lg font-semibold">Kaynağından keşfet</h2><p className="mt-4 text-sm leading-7 text-muted">Bu kayıt resmî ürün ve üretici kaynaklarıyla birlikte yayımlandı. Üretici bağlantısı Türkiye&apos;deki ekibi gösterir; sahiplik veya merkez ülke iddiası değildir.</p><a className="mt-5 block text-sm underline" href={m.source} target="_blank" rel="noreferrer">Ürün kaynağı ↗</a><a className="mt-4 block text-sm underline" href={m.locationSource} target="_blank" rel="noreferrer">Türkiye / şehir kaynağı ↗</a><Link className="mt-5 block text-sm underline" href={"/harita?sehir=" + encodeURIComponent(m.city) + "&q=" + encodeURIComponent(m.name)}>Üreticiyi haritada göster ↗</Link><Link href="/urun-gonder" className="mt-5 block text-xs text-muted underline">Kayıt düzeltmesi öner</Link></aside>
    </div>
