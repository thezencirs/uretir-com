...
export function Footer() {
  ...
  return <footer className="mt-28 bg-[#171916] text-[#f1f2eb]">
    ...
    <div className="section-wrap grid gap-14 py-14 md:grid-cols-[1.5fr_1fr_1fr] md:py-20">
      ...
      <div><p className="eyebrow text-[#848b80]">Keşfet</p><div className="mt-5 grid gap-3 text-sm text-[#c4c8bd]"><Link href="/araclar">Bütün araçlar</Link><Link href="/uygulamalar">Uygulamalar</Link><Link href="/harita">Harita</Link><Link href="/gelismeler">Gelişmeler</Link><Link href="/blog">Tüm yazılar</Link><Link href="/ekosistem">Ekosistem</Link><Link href="/mimar-ai">MimarAI</Link><Link href="/indirim-ai">İndirimAI</Link><Link href="/magazin-ai">MagazinAI</Link></div></div>
      <div><p className="eyebrow text-[#848b80]">Üretir</p><div className="mt-5 grid gap-3 text-sm text-[#c4c8bd]"><Link href="/yonetim">Yönetim</Link><Link href="/hakkimizda">Hakkımızda</Link><Link href="/iletisim">İletişim</Link><Link href="/gizlilik-politikasi">Gizlilik politikası</Link><a href="/feed.xml" {...analyticsAttributes({ event: "rss_select", surface: "footer", target: "feed" })}>RSS akışı</a><a href="mailto:merhaba@uretir.com" className="inline-flex items-center gap-1" {...analyticsAttributes({ event: "contact_select", surface: "footer", target: "email" })}>Bize yazın <ArrowUpRight size={14} /></a></div></div>
    </div>
