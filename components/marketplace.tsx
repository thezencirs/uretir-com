...
export function Marketplace({ mapFirst = false, hero = false }: { mapFirst?: boolean; hero?: boolean }) {
  ...
  return <div className={styles.market}>
    ...
    <div className={styles.container} id="kesfet">
      ...
      <form className={styles.search} action="/ara" method="get"><Search size={23} /><label className="sr-only" htmlFor="product-search">Ürün, üretici veya ihtiyaç ara</label><input id="product-search" name="q" value={query} onChange={(e) => { setQuery(e.target.value); updateUrl("q", e.target.value); }} placeholder="Fotoğraf, İngilizce, oyun, AI, Codeway…" maxLength={120} />{query && <button type="button" aria-label="Aramayı temizle" onClick={() => { setQuery(""); updateUrl("q", ""); }}><X size={18} /></button>}<span>{products.length} ürün</span></form>
