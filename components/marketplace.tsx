...
import { cities, filterProducts, makers, products, tasks } from "@/lib/marketplace";
...
export function Marketplace({ mapFirst = false, hero = false }: { mapFirst?: boolean; hero?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (tasks.includes(params.get("ihtiyac") ?? "")) setTask(params.get("ihtiyac")!);
  ...
  const mapProducts = useMemo(() => filterProducts(query, category, "Tümü", task).filter((p) => !savedOnly || saved.includes(p.slug)), [query, category, task, savedOnly, saved]);
  const results = useMemo(() => {
    const result = mapProducts.filter((p) => city === "Tümü" || makers[p.maker].city === city);
    return sort === "name" ? [...result].sort((a, b) => a.name.localeCompare(b.name, "tr")) : result;
  }, [mapProducts, city, sort]);
  ...
  return <div className={styles.market}>
    {hero && <section className={styles.hero}>
      ...
      <div className={styles.heroBottom}><span>Uygulamalar · Oyunlar · Yapay zekâ · Girişimler</span><span>Görsel: AI editoryal illüstrasyon</span></div>
    </section>}
    <div className={styles.container} id="kesfet">
      ...
      <div className={styles.search}><Search size={23} /><label className="sr-only" htmlFor="product-search">Ürün, üretici veya ihtiyaç ara</label><input id="product-search" value={query} onChange={(e) => { setQuery(e.target.value); updateUrl("q", e.target.value); }} placeholder="Fotoğraf, İngilizce, oyun, AI, Codeway…" maxLength={120} />{query && <button aria-label="Aramayı temizle" onClick={() => { setQuery(""); updateUrl("q", ""); }}><X size={18} /></button>}<span>{products.length} ürün</span></div>
      <div className={styles.taskRail} aria-label="İhtiyaca göre keşfet">{tasks.map((t) => <button key={t} aria-pressed={task === t} onClick={() => { const next = task === t ? "Tümü" : t; setTask(next); updateUrl("ihtiyac", next); }}>{t}<ArrowUpRight size={13} /></button>)}</div>
      <div className={styles.layout}><main className={styles.results}>
        ...
        {view === "map" && <><ProductMap products={mapProducts} city={city} onCity={changeCity} /><div className={styles.cityRail}>{Object.keys(cities).map((c) => <button key={c} aria-pressed={city === c} onClick={() => changeCity(city === c ? "Tümü" : c)}>{c} <span>{mapProducts.filter((p) => makers[p.maker].city === c).length}</span></button>)}</div></>}
        ...
        <div className={styles.cards}>{results.map((p) => { const m = makers[p.maker]; return <article key={p.slug} className={styles.card}>
          ...
          <Link href={"/urun/" + p.slug} className={styles.productTitle}><h3>{p.name}</h3><ArrowUpRight size={17} /></Link><p>{p.description}</p><div className={styles.maker}>{m.name}<span>↗ {m.city}</span></div>
          <div className={styles.cardBottom}><button onClick={() => { const next = task === p.task ? "Tümü" : p.task; setTask(next); updateUrl("ihtiyac", next); }}>{p.task}</button><button disabled={!compare.includes(p.slug) && compare.length >= 3} aria-pressed={compare.includes(p.slug)} aria-label={p.name + " karşılaştır"} onClick={() => setCompare(compare.includes(p.slug) ? compare.filter((s) => s !== p.slug) : [...compare, p.slug])}>{compare.includes(p.slug) ? <Check size={15} /> : <SlidersHorizontal size={15} />}</button></div>
        </article>; })}</div>
        ...
        <p className={styles.note}>06 Eylül 2026 kaynak taraması · {Object.keys(makers).length} üretici, {products.length} ürün, {Object.keys(cities).length} şehir. Genişleyen editoryal dizin; sektörün tamamı veya canlı sıralama değildir. Ürün sayfalarında kaynaklar yer alır.</p>
      </main><aside className={styles.sidebar}>
        <div className={styles.sideTitle}><span className={styles.dot} /> EKOSİSTEM NOTU</div>
        <a className={styles.investment} href="https://hubx.co/news/hubx-point72" target="_blank" rel="noreferrer"><span>28 AĞUSTOS 2026 · İZMİR</span><h3>Türkiye&apos;den<br />dünyaya.</h3><strong>$75M&apos;a kadar</strong><p>HubX, Point72 yatırımını duyurdu: 50 milyon dolar başlangıç yatırımı ve 25 milyon dolar ek yatırım seçeneği.</p><span>Şirket duyurusunu oku ↗</span></a>
        <div className={styles.sideCard}><Map size={25} /><h3>Bir şehir seç.<br />Üretenleri tanı.</h3><p>Ürünlerin arkasındaki ekipleri Türkiye haritasında keşfet.</p><Link href="/map">Map&apos;i aç <ArrowUpRight size={17} /></Link></div>
        <div className={styles.sideCard}><span className={styles.eyebrow}>ÜRETİR EKOSİSTEMİ</span><Link href="/puan-ai">PuanAI <ArrowUpRight size={16} /></Link><Link href="/araclar">AI bilgi merkezleri <ArrowUpRight size={16} /></Link><Link href="/blog">Journal / Makaleler <ArrowUpRight size={16} /></Link><Link href="/ekosistem">Bütün özellikler <ArrowUpRight size={16} /></Link></div>
        <div className={styles.makerList}><h3>Üreten ekipler</h3>{Object.entries(makers).map(([id, m]) => <button key={id} onClick={() => { reset(); setQuery(m.name); updateUrl("q", m.name); }}><span style={{ background: m.color }}>{m.name.slice(0, 1)}</span><strong>{m.name}<small>{m.city}</small></strong><span>{products.filter((p) => p.maker === id).length} ↗</span></button>)}</div>
      </aside></div>
      {compare.length > 0 && <section className={styles.comparison} aria-label="Ürün karşılaştırma"><div><h2>Yan yana keşfet <small>{compare.length}/3</small></h2><button aria-label="Karşılaştırmayı temizle" onClick={() => setCompare([])}><X /></button></div><p>En fazla 3 ürün seç. Fiyat ve mağaza bilgileri resmî sayfalarda.</p><div className={styles.compareGrid}>{compare.map((slug) => { const p = products.find((p) => p.slug === slug)!; return <article key={slug}><h3>{p.name}</h3><p>{makers[p.maker].name} · {makers[p.maker].city}</p><p>{p.category} / {p.task}</p><p>{p.description}</p><Link href={"/urun/" + slug}>Ayrıntılar ↗</Link><button aria-label={p.name + " karşılaştırmadan kaldır"} onClick={() => setCompare(compare.filter((s) => s !== slug))}>Kaldır</button></article>; })}</div></section>}
