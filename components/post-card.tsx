...
export function PostCard({ post, featured = false, reviewMode = false }: { post: Post; featured?: boolean; reviewMode?: boolean }) {
  const visibleAuthor = reviewMode ? "Üretir Editoryal" : post.author;
  const initials = visibleAuthor.split(" ").map((part) => part[0]).join("").slice(0, 2);
