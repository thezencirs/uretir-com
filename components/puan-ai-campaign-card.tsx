...
import { getPuanAICampaignProvenance, type PuanAIBank, type PuanAICampaign } from "@/lib/puan-ai";
...
export function PuanAICampaignCard({ campaign, bank, compact = false }: { campaign: PuanAICampaign; bank?: PuanAIBank; compact?: boolean }) {
  ...
  const verified = provenance.mode === "verified";
  return <article className={`puan-campaign-card${compact ? " puan-campaign-card--compact" : ""}`}>
    <div className="puan-campaign-card__topline">
      ...
      <span className="puan-campaign-card__updated">{verified ? `Doğrulandı · ${campaign.updatedAt}` : provenance.label}</span>
    </div>
    ...
    <div className="puan-campaign-card__details">
      ...
      <span><CalendarDays size={13} /> {verified ? campaign.expiresAt : "Gerçek tarih yok"}</span>
    </div>
