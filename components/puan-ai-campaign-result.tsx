...
export function PuanAICampaignResult({ campaign, compact = false }: CampaignResultProps) {
  ...
  const decision = "decision" in campaign ? campaign.decision : null;
  ...
  return <article className={`pa-campaign-card ${compact ? "pa-campaign-card--compact" : ""}`}>
    ...
    <div className="pa-campaign-card__body">
      ...
      {decision && <dl className="pa-campaign-facts">
        <div><dt>PuanAI skoru</dt><dd>{decision.score}/100</dd></div>
        <div><dt>İşlem tutarı</dt><dd>{decision.price === null ? "Doğrulanamadı" : `${decision.price.toLocaleString("tr-TR")} TL`}</dd></div>
        <div><dt>Efektif nakit maliyeti</dt><dd>{decision.effectiveCashCost === null ? "Doğrulanamadı" : `${decision.effectiveCashCost.toLocaleString("tr-TR")} TL`}</dd></div>
        {decision.rewardValue > 0 && <div><dt>Ödül / puan değeri</dt><dd>{decision.rewardValue.toLocaleString("tr-TR")} TL karşılığı — doğrudan indirim değildir</dd></div>}
      </dl>}
