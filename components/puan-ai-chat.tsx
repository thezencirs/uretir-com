...
export function PuanAIChat() {
  ...
  async function sendMessage(value = input) {
    ...
    try {
      ...
      setMessages((items) => items.map((item) => item.id === assistantId
        ? { ...item, content: "Güncel kampanya bilgisini doğrulayamadım. Bu nedenle kesin bir avantaj rakamı vermiyorum.", pending: false }
        : item));
    } finally {
      setStreaming(false);
      void loadHistory();
  ...
  return <div className="pa-chat-shell" id="danisman">
